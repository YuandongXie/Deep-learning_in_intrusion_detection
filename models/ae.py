import logging
import os
from tensorflow_core.python.keras.layers import Input, Dense, Dropout, Activation
...
class AE:
    ...
    def fit(self, X_train, y_train=None, X_valid=None, y_valid=None):
        ...
        # --------------------------------
        # Create callbacks
        self.params['tensorboard_log_dir']=os.path.join(self.params['tensorboard_log_dir'])
        tensorboard_cb = TensorBoard(log_dir=self.params['tensorboard_log_dir'], batch_size=batch_size)

        interval = epochs#max(epochs // 10, 10)
