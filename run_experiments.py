...
def create_tensorboard_log_dir(results_dir):#dir is short for directory
    log_dir = results_dir + '/tf_logs_run_' + time.strftime("%Y_%m_%d-%H_%M_%S")
    log_dir = os.path.join(results_dir , 'tf_logs_run_' , time.strftime("%Y_%m_%d-%H_%M_%S"))
    os.makedirs(log_dir)
...
def train_model(model, exp_params, datasets, is_first_time=True):
    ...
    model_filepath = os.path.join(exp_params['results_dir'] , 'best_model.pickle')
